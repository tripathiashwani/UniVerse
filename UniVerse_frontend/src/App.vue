<template>
  ...
  <GoogleLogin :callback="callback" prompt auto-login />
            ...
            <RouterLink to="/login" class="py-2 px-4 bg-gray-600 text-white rounded-lg hover:bg-gray-700">Log in
            </RouterLink>
            <RouterLink to="/signup" class="py-2 px-4 bg-purple-600 text-white rounded-lg hover:bg-purple-700">Sign up
            </RouterLink>
          </div>
          <button @click="toggleMobileMenu"
            class="md:hidden text-gray-600 hover:text-purple-700 focus:outline-none ml-4">
    ...
    <transition enter-active-class="transition duration-200 ease-out" enter-from-class="transform scale-95 opacity-0"
      ...
      <div v-if="isMobileMenuOpen" class="md:hidden mt-4 space-y-4">
        ...
        <div v-else class="flex flex-col space-y-4">
          ...
          <RouterLink to="/login" class="py-2 px-4 bg-gray-600 text-white rounded-lg hover:bg-gray-700 text-center">Log
            in</RouterLink>
          <RouterLink to="/signup"
            class="py-2 px-4 bg-purple-600 text-white rounded-lg hover:bg-purple-700 text-center">Sign up</RouterLink>
        </div>
      </div>
    </transition>
  </nav>
...
const callback = async (response) => {
  try {
    ...
    const loginResponse = await axios.post('/api/google_login/', { token })

    const { access, refresh } = loginResponse.data.tokens
    userStore.setToken(loginResponse.data.tokens)
